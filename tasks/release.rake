...
namespace :custom do
  task :release, [:version, :type, :quiet] do |_t, args|
    ...
    raise "Please provide a valid release type: major, minor, or patch" unless valid_type?(args[:type])
...
namespace :major do
  task :release do
    ...
    Rake::Task['custom:release'].invoke(new_major_version, 'major', 'true')
  end
end
...
namespace :minor do
  task :release do
    ...
    Rake::Task['custom:release'].invoke(new_minor_version, 'minor', 'true')
  end
end
...
namespace :patch do
  task :release do
    ...
    Rake::Task['custom:release'].invoke(new_patch_version, 'patch', 'true')
  end
end